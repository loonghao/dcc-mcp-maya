...
def _load_script(skill_dir, script_name):
    ...
    spec = importlib.util.spec_from_file_location(
        "r21_{}_{}".format(skill_dir.replace("-", "_"), script_name),
        str(script_path),
    )
...
class TestListExpressions:
    ...
    def test_list_nodes(self):
        ...
        mc.objExists.return_value = True
        mc.expression.side_effect = lambda node, **kw: (
            "" if kw.get("object") else "pSphere1.tx = sin(time);"
        )
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-expressions", "list_expressions")
            result = mod.list_expressions()
    ...
    def test_list_with_filter(self):
        ...
        mc.objExists.return_value = True
        mc.expression.side_effect = lambda node, **kw: (
            "" if kw.get("object") else "pCube1.tx = 0;"
        )
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-expressions", "list_expressions")
...
class TestEditExpression:
    def test_node_not_found(self):
        ...
        mc.objExists.return_value = False
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-expressions", "edit_expression")
...
class TestImportMocap:
    def test_missing_file_path(self):
        mock_maya, mc = _make_mock_maya()
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-mocap", "import_mocap")
...
class TestCreateHikDefinition:
    def test_missing_character_name(self):
        ...
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-mocap", "create_hik_definition")
    ...
    def test_create_success(self):
        mock_maya, mc = _make_mock_maya()
        mock_mel = MagicMock()
        mock_mel.eval.side_effect = lambda expr: (
            "HIKChar1" if "hikCreateCharacter" in expr else None
        )
        mc.objExists.return_value = True
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-mocap", "create_hik_definition")
            result = mod.create_hik_definition(
                "myChar", {"Hips": "Hips_jnt", "Spine": "Spine_jnt"}
            )
        assert result["success"] is True
    ...
    def test_unknown_slot_skipped(self):
        ...
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-mocap", "create_hik_definition")
            result = mod.create_hik_definition(
                "myChar", {"Hips": "Hips_jnt", "UnknownSlot": "some_jnt"}
            )
        assert result["success"] is True
...
class TestBakeMocapToRig:
    def test_missing_params(self):
        ...
        mock_mel = MagicMock()
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-mocap", "bake_mocap_to_rig")
    ...
    def test_bake_success(self):
        ...
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-mocap", "bake_mocap_to_rig")
            result = mod.bake_mocap_to_rig(
                "srcChar", "tgtChar", start_frame=1, end_frame=100
            )
        assert result["success"] is True
...
class TestCleanMocapKeys:
    def test_no_joints_in_scene(self):
        ...
        mc.ls.return_value = []
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-mocap", "clean_mocap_keys")
...
class TestApplyMuscleSkin:
    def test_mesh_not_found(self):
        ...
        mc.objExists.return_value = False
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc, "maya.mel": mock_mel}):
            mod = _load_script("maya-muscle", "apply_muscle_skin")
...
class TestCreateAssemblyDefinition:
    def test_create_default(self):
        ...
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-scene-assembly", "create_assembly_definition")
...
class TestAddAssemblyRepresentation:
    def test_invalid_rep_type(self):
        ...
        mc.objExists.return_value = True
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-scene-assembly", "add_assembly_representation")
    ...
    def test_add_scene_rep_with_file(self):
        ...
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            ...
            result = mod.add_assembly_representation(
                "asm1", "Scene", file_path="/path/to/scene.ma"
            )
        assert result["success"] is True
...
class TestCreateAssemblyReference:
    def test_missing_definition(self):
        mock_maya, mc = _make_mock_maya()
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-scene-assembly", "create_assembly_reference")
...
class TestListAssemblies:
    def test_list_empty(self):
        ...
        mc.ls.return_value = []
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-scene-assembly", "list_assemblies")
    ...
    def test_list_definitions_only(self):
        ...
        mc.ls.side_effect = lambda type=None, **kw: (
            ["asm1"] if type == "assemblyDefinition" else []
        )
    ...
    def test_list_all(self):
        mock_maya, mc = _make_mock_maya()
        mc.ls.side_effect = lambda type=None, **kw: (
            ["asm1"] if type == "assemblyDefinition"
            else (["ref1"] if type == "assemblyReference" else [])
        )
...
class TestCreateProxy:
    def test_source_not_found(self):
        ...
        mc.objExists.return_value = False
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "create_proxy")
...
class TestSwapProxy:
    def test_proxy_not_found(self):
        ...
        mc.objExists.return_value = False
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "swap_proxy")
    ...
    def test_show_proxy_true(self):
        ...
        mc.getAttr.side_effect = lambda attr, **kw: (
            "pSphere1" if "proxySource" in attr else False
        )
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "swap_proxy")
    ...
    def test_show_proxy_false(self):
        ...
        mc.attributeQuery.return_value = True
        mc.getAttr.side_effect = lambda attr, **kw: (
            "pSphere1" if "proxySource" in attr else True
        )
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "swap_proxy")
...
class TestListProxies:
    def test_list_empty(self):
        ...
        mc.ls.return_value = []
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "list_proxies")
...
class TestSetProxyAttribute:
    def test_proxy_not_found(self):
        ...
        mc.objExists.return_value = False
        with patch.dict(sys.modules, {"maya": mock_maya, "maya.cmds": mc}):
            mod = _load_script("maya-proxy-mesh", "set_proxy_attribute")
