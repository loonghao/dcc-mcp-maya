...
from conftest import SKILLS_ROOT, load_and_call, load_and_call_with_mel
...
class TestImportAudio:
    ...
    def test_file_not_found(self, tmp_path):
        mc = self._mk()
        result = _call("maya-audio/scripts/import_audio.py", mc,
    ...
    def test_named_with_offset(self, tmp_path):
        ...
        mc = self._mk()
        result = _call("maya-audio/scripts/import_audio.py", mc,
                       file_path=str(wav), name="my_sound", offset=10.0)
        assert result["success"] is True
...
class TestSetTimelineAudio:
    def test_missing_node(self):
        ...
        result = _call_mel("maya-audio/scripts/set_timeline_audio.py", mc,
                           sound_node="missing_snd")
        assert result["success"] is False
        assert "not found" in result["message"].lower()
    ...
    def test_happy_path(self):
        ...
        result = _call_mel("maya-audio/scripts/set_timeline_audio.py", mc,
                           mock_mel=mock_mel, sound_node="snd1")
        assert result["success"] is True
...
class TestCreateGeometryCache:
    def test_missing_object(self, tmp_path):
        ...
        mc.objExists.return_value = False
        mock_mel = MagicMock()
        result = _call_mel("maya-cache/scripts/create_geometry_cache.py", mc,
                           mock_mel=mock_mel,
                           objects=["ghost_mesh"],
    ...
    def test_playback_defaults_used(self, tmp_path):
        mc = MagicMock()
        mc.objExists.return_value = True
        mc.playbackOptions.side_effect = lambda query, minTime=False, maxTime=False: (
            1.0 if minTime else 48.0
        )
        mc.ls.return_value = []
        mock_mel = MagicMock()
        result = _call_mel("maya-cache/scripts/create_geometry_cache.py", mc,
    ...
    def test_prompt_present(self, tmp_path):
        ...
        mc.ls.return_value = []
        result = _call_mel("maya-cache/scripts/create_geometry_cache.py", mc,
                           objects=["mesh1"], directory=str(tmp_path),
                           start_frame=1.0, end_frame=10.0)
        assert "prompt" in result and result["prompt"]
...
class TestAttachGeometryCache:
    def test_missing_mesh(self, tmp_path):
        ...
        mc = MagicMock()
        mc.objExists.return_value = False
        result = _call_mel("maya-cache/scripts/attach_geometry_cache.py", mc,
                           mesh="missing_mesh",
    ...
    def test_prompt_present(self, tmp_path):
        ...
        mc.ls.return_value = []
        result = _call_mel("maya-cache/scripts/attach_geometry_cache.py", mc,
                           mesh="pSphere1", cache_xml_path=str(xml))
        assert "prompt" in result and result["prompt"]
...
class TestDeleteGeometryCache:
    def test_missing_cache_node(self):
        mc = MagicMock()
        mc.objExists.return_value = False
        result = _call("maya-cache/scripts/delete_geometry_cache.py", mc,
                       cache_node="ghost_cf")
        assert result["success"] is False
    ...
    def test_happy_path_no_delete_files(self):
        ...
        mc.objExists.return_value = True
        result = _call("maya-cache/scripts/delete_geometry_cache.py", mc,
                       cache_node="cf1", delete_files=False)
        assert result["success"] is True
    ...
    def test_delete_files_removes_on_disk(self, tmp_path):
        ...
        result = _call("maya-cache/scripts/delete_geometry_cache.py", mc,
                       cache_node="cf1", delete_files=True)
        assert result["success"] is True
...
class TestCreateOcean:
    def _mk(self):
        ...
        mc.sets.side_effect = lambda *a, **kw: (
            "ocean_surface_SG" if kw.get("empty") else None
        )
        return mc
...
class TestSetOceanAttribute:
    def test_missing_shader(self):
        mc = MagicMock()
        mc.objExists.return_value = False
        result = _call("maya-ocean/scripts/set_ocean_attribute.py", mc,
                       shader="ghost_shader", attribute="waveHeight", value=2.0)
        assert result["success"] is False
    ...
    def test_happy_path(self):
        ...
        mc.objExists.return_value = True
        result = _call("maya-ocean/scripts/set_ocean_attribute.py", mc,
                       shader="oceanShader1", attribute="waveHeight", value=3.5)
        assert result["success"] is True
    ...
    def test_context_keys(self):
        ...
        mc.objExists.return_value = True
        result = _call("maya-ocean/scripts/set_ocean_attribute.py", mc,
                       shader="sh1", attribute="windSpeed", value=10.0)
    ...
    def test_prompt_present(self):
        ...
        mc.objExists.return_value = True
        result = _call("maya-ocean/scripts/set_ocean_attribute.py", mc,
                       shader="sh1", attribute="waveHeight", value=1.0)
        assert "prompt" in result and result["prompt"]
...
class TestAddOceanWake:
    def test_missing_shader(self):
        mc = MagicMock()
        mc.objExists.return_value = False
        result = _call("maya-ocean/scripts/add_ocean_wake.py", mc,
                       shader="ghost_sh")
        assert result["success"] is False
    ...
    def test_happy_path_no_wake_object(self):
        ...
        result = _call("maya-ocean/scripts/add_ocean_wake.py", mc,
                       shader="oceanShader1", wake_size=2.0)
        assert result["success"] is True
    ...
    def test_with_wake_object_exists(self):
        ...
        result = _call("maya-ocean/scripts/add_ocean_wake.py", mc,
                       shader="sh", wake_object="boat", wake_size=1.0)
        assert result["success"] is True
    ...
    def test_with_wake_object_missing_skips_constraint(self):
        ...
        mc.spaceLocator.return_value = ["sh_wake_loc"]
        result = _call("maya-ocean/scripts/add_ocean_wake.py", mc,
                       shader="sh", wake_object="ghost_boat", wake_size=1.0)
        assert result["success"] is True
...
class TestServerSearchAPI:
    ...
    def test_search_skills_delegates_to_registry(self):
        ...
        mock_registry.search_actions.assert_called_once_with(
            category="geometry", tags=["create"], dcc_name="maya"
        )
